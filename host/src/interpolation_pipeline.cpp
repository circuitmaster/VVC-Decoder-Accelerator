...
int main(int argc, char** argv) {
    ...
    try {
        ...
        // inserted before the file extension so each scenario writes its own file.
        // Returns nullptr if streaming is not applicable or GStreamer fails to start.
        auto make_streamer = [&](const std::string& file_suffix)
            -> std::unique_ptr<FrameStreamer>
        {
            if (input_cfg.mode != InputMode::Video) return nullptr;
            ...
            auto s = std::make_unique<FrameStreamer>(
...

        {
            ...
            run_scenario_2(kernel, input_bo, output_bo, input_ptr, output_ptr, cfg, run_cfg, input_cfg, yuv_ptr, s2.get());
            if (s2) s2->stop();
        }

        {
            auto s3 = make_streamer("_s3");
            run_scenario_3(kernel, device, g_in, g_out, cfg, run_cfg, input_cfg, yuv_ptr, s3.get());
            if (s3) s3->stop();
        }
