...
public:
    ...
    void run(input_buffer<TT_DATA, extents<BLOCK_SIZE*BLOCK_SIZE>>& __restrict in,
...
public:
    ...
    // Same tile dims as horizontal: mmul<4,8,4>
    // Two consecutive 4×4 H^T tiles are zipped into one 4×8 A tile
    static constexpr int TILEdM = 4;
    static constexpr int TILEdK = 8;
    static constexpr int TILEdN = 4;

    // Input tile size from H kernel output (4×4 transposed tiles)
    static constexpr int IN_TILEdK = 4;
...
public:
    ...
    void run(input_buffer<TT_DATA, extents<BLOCK_SIZE*BLOCK_SIZE>>& __restrict x_transform,
