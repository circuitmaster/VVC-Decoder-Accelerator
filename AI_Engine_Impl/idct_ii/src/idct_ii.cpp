...
void idct_ii_b64_horizontal_transform_static::run(
    input_buffer<TT_DATA, extents<WINDOW_SIZE>>& __restrict in,
...
void idct_ii_b64_vertical_transform_static::run(
    input_buffer<TT_DATA, extents<WINDOW_SIZE>>& __restrict x_transform,
    ...
    // intermediate 1D result tile descriptor to read 4×4 tiles
    auto tdsc_Transx = aie::make_tensor_descriptor<TT_DATA, TILEdM*TILEdK>(
        aie::tensor_dim((BLOCK_SIZE/TILEdM)/UNROLL_M, (BLOCK_SIZE/TILEdK)*UNROLL_M),  // X_group outer: 8, stride=32
        aie::tensor_dim((BLOCK_SIZE/TILEdN)/UNROLL_N, 0),                                  // Y dummy: 4, stride=0
        aie::tensor_dim(BLOCK_SIZE/TILEdK, 1),                                          // K_tile_4: 16, stride=1
        aie::tensor_dim(UNROLL_M, BLOCK_SIZE/TILEdK)                                    // parallel X: 2, stride=16
    );

    // Coeff tile descriptor to read 4x8 tiles
    ...
    // Output descriptor: result is 4×8, write with transposed tiles
    ...
    for (int mn = 0; mn < (BLOCK_SIZE*BLOCK_SIZE)/((TILEdM*UNROLL_M)*(TILEdN*UNROLL_N)); ++mn)
    {
        ...
        // K loop: 16 iterations (BLOCK_SIZE/TILEdK = 64/4)
        for (int kk = 0; kk < (BLOCK_SIZE/TILEdK); ++kk)
        {
            // pop inner stream from input and coeff buffer
            auto Transx_inner = tTransx.pop();
            auto Coeffs_inner = tCoeffs.pop();

            // stage operand tiles in vector registers
            aie::vector<TT_DATA, MMUL_V::size_A> vTransx[UNROLL_M];
            for (int m = 0; m < UNROLL_M; ++m)
                chess_unroll_loop(*)
            {
                Transx_inner >> vTransx[m];
            }
