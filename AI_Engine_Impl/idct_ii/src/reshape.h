...
// std::array<adf::tiling_parameters, NUM_CU> gen_gmio_write_in_patterns() {
//     std::array<adf::tiling_parameters, NUM_CU> p;
//     for (int i = 0; i < NUM_CU; ++i) {
//         p[i] = {
//             .buffer_dimension = {KernelT::BLOCK_SIZE, N_WINDOWS * KernelT::BATCH_SIZE * NUM_CU},
...
// std::array<adf::tiling_parameters, NUM_CU> gen_frame_in_buffer_read_patterns() {
//     std::array<adf::tiling_parameters, NUM_CU> p;
//     for (int i = 0; i < NUM_CU; ++i) {
//         p[i] = {
//             .buffer_dimension = {KernelT::BLOCK_SIZE, N_WINDOWS * KernelT::BATCH_SIZE * NUM_CU},
...
// std::array<adf::tiling_parameters, NUM_CU> gen_frame_out_buffer_write_patterns() {
//     std::array<adf::tiling_parameters, NUM_CU> p;
//     for (int i = 0; i < NUM_CU; ++i) {
//         p[i] = {
//             .buffer_dimension = {KernelT::BLOCK_SIZE, N_WINDOWS * KernelT::BATCH_SIZE * NUM_CU},
...
// std::array<adf::tiling_parameters, NUM_CU> gen_gmio_read_out_patterns() {
//     std::array<adf::tiling_parameters, NUM_CU> p;
//     for (int i = 0; i < NUM_CU; ++i) {
//         p[i] = {
//             .buffer_dimension = {KernelT::BLOCK_SIZE, N_WINDOWS * KernelT::BATCH_SIZE * NUM_CU},
...
// GMIO parallelism: 0 → 1 port, 1 → 2 ports, 2 → 4 ports per engine
// Defined via Makefile: -DGMIO_LEVEL=N
#ifndef GMIO_LEVEL
#define GMIO_LEVEL 0
#endif
static constexpr int NUM_GMIO_PORTS = 1 << GMIO_LEVEL;
static_assert((NUM_CU * N_WINDOWS_2D) % NUM_GMIO_PORTS == 0,
              "NUM_CU * N_WINDOWS_2D must be divisible by NUM_GMIO_PORTS");

// single port

// GMIO → shared_buf_in: linear write, per-core offset
std::array<adf::tiling_parameters, NUM_CU> gen_2d_gmio_write_in() {
    std::array<adf::tiling_parameters, NUM_CU> p;
    for (int i = 0; i < NUM_CU; ++i) {
        p[i] = {
            .buffer_dimension = {(int)(NUM_CU * N_WINDOWS_2D * BLOCK_ELEMS)},
            .tiling_dimension = {1},
...
std::array<adf::tiling_parameters, NUM_CU> gen_2d_shared_in_read() {
    std::array<adf::tiling_parameters, NUM_CU> p;
    for (int i = 0; i < NUM_CU; ++i) {
        p[i] = {
            .buffer_dimension = {(int)(NUM_CU * N_WINDOWS_2D * BLOCK_ELEMS)},
...
std::array<adf::tiling_parameters, NUM_CU> gen_2d_shared_mid_write() {
    std::array<adf::tiling_parameters, NUM_CU> p;
    for (int i = 0; i < NUM_CU; ++i) {
        p[i] = {
            .buffer_dimension = {(int)(NUM_CU * N_WINDOWS_2D * BLOCK_ELEMS)},
...
std::array<adf::tiling_parameters, NUM_CU> gen_2d_shared_mid_read() {
    std::array<adf::tiling_parameters, NUM_CU> p;
    for (int i = 0; i < NUM_CU; ++i) {
        p[i] = {
            .buffer_dimension = {(int)(NUM_CU * N_WINDOWS_2D * BLOCK_ELEMS)},
...
std::array<adf::tiling_parameters, NUM_CU> gen_2d_shared_out_write() {
    std::array<adf::tiling_parameters, NUM_CU> p;
    for (int i = 0; i < NUM_CU; ++i) {
        p[i] = {
            .buffer_dimension = {BLOCK, (int)(N_WINDOWS_2D * BLOCK * NUM_CU)},
...
std::array<adf::tiling_parameters, NUM_CU> gen_2d_gmio_read_out() {
    std::array<adf::tiling_parameters, NUM_CU> p;
    for (int i = 0; i < NUM_CU; ++i) {
        p[i] = {
            .buffer_dimension = {BLOCK, (int)(N_WINDOWS_2D * BLOCK * NUM_CU)},
...
// ============================================================
// Multi-port GMIO patterns: NUM_GMIO_PORTS ports per engine
// Each port handles a contiguous 1/NUM_GMIO_PORTS slice of the data.
// When GMIO_LEVEL=0 (NUM_GMIO_PORTS=1), this reduces to the single-port case.
// ============================================================

// GMIO[g] → shared_buf_in: each port writes a contiguous chunk (linear)
std::array<adf::tiling_parameters, NUM_GMIO_PORTS> gen_2d_gmio_write_in_ports() {
    static constexpr int TOTAL = NUM_CU * N_WINDOWS_2D * BLOCK_ELEMS;
    static constexpr int PER_PORT = TOTAL / NUM_GMIO_PORTS;
    std::array<adf::tiling_parameters, NUM_GMIO_PORTS> p;
    for (int i = 0; i < NUM_GMIO_PORTS; ++i) {
        p[i] = {
            .buffer_dimension = {TOTAL},
            .tiling_dimension = {1},
            .offset = {i * PER_PORT},
            .tile_traversal = {
                {.dimension = 0, .stride = 1, .wrap = PER_PORT}
            }
        };
    }
    return p;
...
// shared_buf_out → GMIO[g]: each port reads a contiguous column range
std::array<adf::tiling_parameters, NUM_GMIO_PORTS> gen_2d_gmio_read_out_ports() {
    static constexpr int TOTAL_DIM1 = N_WINDOWS_2D * BLOCK * NUM_CU;
    static constexpr int PER_PORT_DIM1 = TOTAL_DIM1 / NUM_GMIO_PORTS;
    std::array<adf::tiling_parameters, NUM_GMIO_PORTS> p;
    for (int i = 0; i < NUM_GMIO_PORTS; ++i) {
        p[i] = {
            .buffer_dimension = {BLOCK, TOTAL_DIM1},
            .tiling_dimension = {1, 1},
            .offset = {0, i * PER_PORT_DIM1},
            .tile_traversal = {
                {.dimension = 0, .stride = 1, .wrap = BLOCK},
                {.dimension = 1, .stride = 1, .wrap = PER_PORT_DIM1}
            }
        };
    }
    return p;
