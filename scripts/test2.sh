#!/usr/bin/env bash
# test2.sh - sweep parameters for main2 and capture timing to CSV.
...
total_runs=$(( ${#BLOCK_SIZES[@]} * ${#SIZES[@]} * ${#SELECTORS[@]} * RUNS ))
run_idx=0
show_progress() {
    run_idx=$((run_idx + 1))
    printf "\r[%d/%d] bs=%s sz=%s sel=%s" "$run_idx" "$total_runs" "$bs" "$sz" "$sel"
}
trap '[[ $BASH_COMMAND == *"./main2 "* ]] && show_progress' DEBUG
...
CSV_OUT="timings_comb.csv"

echo "block_size,size,selector,total_mean,total_median,total_variance,input_mean,input_median,input_variance,kernel_mean,kernel_median,kernel_variance,output_mean,output_median,output_variance" > "${CSV_OUT}"
    ...
    for sz in "${SIZES[@]}"; do
        for sel in "${SELECTORS[@]}"; do
            ...
            for ((run=1; run<=RUNS; run++)); do
                OUTPUT=$(./main2 "${XCLBIN}" "${bs}" "${sz}" "${SHIFT}" "${OUTPUT_MIN}" "${OUTPUT_MAX}" "${SKIP_IN_LINES}" "${sel}")
            ...
            echo "${bs},${sz},${sel},${total_stats},${input_stats},${kernel_stats},${output_stats}" >> "${CSV_OUT}"
        done
    done
done
