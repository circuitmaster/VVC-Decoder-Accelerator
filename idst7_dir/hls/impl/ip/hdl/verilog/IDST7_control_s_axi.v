...
#(parameter
    ...
    output wire [31:0]                   oMin,
    output wire [31:0]                   oMax,
...
localparam
    ...
    ADDR_OUT_R_DATA_0      = 7'h1c,
    ADDR_OUT_R_DATA_1      = 7'h20,
    ADDR_OUT_R_CTRL        = 7'h24,
    ADDR_BLOCK_SIZE_DATA_0 = 7'h28,
    ADDR_BLOCK_SIZE_CTRL   = 7'h2c,
    ADDR_SIN_DATA_0        = 7'h30,
    ADDR_SIN_CTRL          = 7'h34,
    ADDR_SIZE_DATA_0       = 7'h38,
    ADDR_SIZE_CTRL         = 7'h3c,
    ADDR_SHIFT_DATA_0      = 7'h40,
    ADDR_SHIFT_CTRL        = 7'h44,
    ADDR_OMIN_DATA_0       = 7'h48,
    ADDR_OMIN_CTRL         = 7'h4c,
    ADDR_OMAX_DATA_0       = 7'h50,
    ADDR_OMAX_CTRL         = 7'h54,
    WRIDLE                 = 2'd0,
    WRDATA                 = 2'd1,
    WRRESP                 = 2'd2,
    WRRESET                = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_oMin = 'b0;
    reg  [31:0]                   int_oMax = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_OMIN_DATA_0: begin
                    rdata <= int_oMin[31:0];
                end
                ADDR_OMAX_DATA_0: begin
                    rdata <= int_oMax[31:0];
                end
            endcase
        end
...
assign oMin          = int_oMin;
assign oMax          = int_oMax;
// int_interrupt
always @(posedge ACLK) begin
    if (ARESET)
        int_interrupt <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        int_block_size[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_BLOCK_SIZE_DATA_0)
            int_block_size[31:0] <= (WDATA[31:0] & wmask) | (int_block_size[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_SIN_DATA_0)
            int_sIn[31:0] <= (WDATA[31:0] & wmask) | (int_sIn[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_OMIN_DATA_0)
            int_oMin[31:0] <= (WDATA[31:0] & wmask) | (int_oMin[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_OMAX_DATA_0)
            int_oMax[31:0] <= (WDATA[31:0] & wmask) | (int_oMax[31:0] & ~wmask);
    end
end
